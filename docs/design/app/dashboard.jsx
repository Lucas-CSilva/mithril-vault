...
function HeroBalance() {
  ...
  const kpis = [
    ...
    { label: 'Investido no mês',value: hero.investidoMes,pos: true,  spark: [2,3,3,4,5,5,6,7],   color: 'var(--frost-pine)' },
    ...
    <div className="card fade-in" style={{ padding: 0, overflow: 'hidden', display: 'grid', gridTemplateColumns: 'minmax(280px, 0.95fr) 1.25fr' }}>
      ...
      <div style={{ padding: '26px 30px', borderRight: '1px solid var(--line)', position: 'relative',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 3, marginTop: 14, color: 'var(--ink)' }}>
          ...
          <span className="num" style={{ fontSize: 46, fontWeight: 700, letterSpacing: '-.03em', lineHeight: 1 }}>
            {hidden ? '••••••' : p.int}
          </span>
          {!hidden && <span className="num" style={{ fontSize: 24, fontWeight: 600, color: 'var(--ink-3)' }}>,{p.dec}</span>}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr' }}>
        {kpis.map((k, i) => (
          <div key={i} style={{
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2.5, borderRadius: '0 0 3px 3px', background: k.color, opacity: 0.55 }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <span style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 500, paddingTop: 4 }}>{k.label}</span>
              <Sparkline data={k.spark} color={k.color} width={54} height={24} />
            </div>
            <div>
              <div className="num" style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.025em', color: 'var(--ink)', lineHeight: 1 }}>
                {k.label === 'Despesas' ? '− ' : ''}{MV.fmtBRL(k.value, {cents:false})}
              </div>
              <div className="mono" style={{ fontSize: 10, color: k.color, fontWeight: 600, marginTop: 5, letterSpacing: '.04em' }}>
